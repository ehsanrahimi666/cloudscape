...
Third, and most importantly, a cloud percentage is not an answer. A phenologist told that a site averages 42 % cloud cover cannot determine whether green-up will be retrievable there. That determination requires knowing when the clear observations fall relative to the phenological transition, how long the gaps are, and what error the resulting sampling induces. The requirement is not confined to phenology as such: phenology-based classification of crops and cropping systems depends on resolving specific transitions within the season (Qiu et al., 2015; Waldner et al., 2015; Liu et al., 2018; Tian et al., 2019), so the same observation pattern that determines whether green-up is retrievable also determines whether a crop map is achievable.
...
**Cloud-removal and reconstruction methods** form a related family that accepts the mask and attempts to recover the obscured surface, for example by blending information from adjacent dates (Hu et al., 2020). These are complementary to detection rather than alternatives to it, and their success depends on the same underlying quantity this paper addresses: how many uncontaminated observations exist nearby in time.
...
Consistency across a reprocessed archive is a distinct concern from mask accuracy. Qiu et al. (2018) evaluated the temporal consistency of Landsat Analysis Ready Data and proposed improvements to resampling, cloud and shadow detection, bidirectional reflectance correction and topographic correction, finding that an updated cloud and shadow detection method moderately improved consistency.

Other widely used chains include the Landsat 8 surface reflectance algorithm LaSRC, which handles aerosol and cloud effects, and FORCE, an end-to-end Landsat and Sentinel-2 framework covering cloud and shadow masking, compositing and time-series analysis.
...
Statistics are aggregated onto EASE-Grid 2.0 global (EPSG:6933), a cylindrical equal-area projection on the WGS84 ellipsoid with standard parallel 30° (Brodzik et al., 2012, 2014). Every cell has identical true ground area by construction, so counts are comparable across latitude and across sensors with different footprint geometries.
...
Every work cited in the text appears in the reference list above, and every
entry in the reference list is cited in the text. Entries were taken either
from the peer-reviewed reference list of Rahimi and Jung (2024b) or verified
against primary sources during preparation. Software and processing chains
discussed in Section 2 without a formal citation (Sen2Cor, MAJA, LaSRC, FORCE,
s2cloudless, CFMask) are described by name as products rather than cited as
publications; if the target journal requires citations for these, the
corresponding methods papers should be added at revision.
